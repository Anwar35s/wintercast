...
from services.security import (
    check_honeypot, check_blocked, check_suspicious_headers,
    get_client_ip, log_request, log_security_event,
    REQUEST_LOG, BLOCKED_IPS, SUSPICIOUS_IPS
)
...
app = FastAPI(title="Wintercast API", version="1.0.0", docs_url=None, redoc_url=None)
...
async def honeypot(request: Request):
    ip = get_client_ip(request)
    log_security_event("HONEYPOT_ROUTE_HIT", ip, {"path": str(request.url.path)})
    return JSONResponse(status_code=404, content={"detail": "Not found"})
...
def stats(key: str = ""):
    ...
    return {
        ...
        "suspicious_ips": dict(list(SUSPICIOUS_IPS.items())[:20]),
        "recent_requests": REQUEST_LOG[-50:],
        "total_logged": len(REQUEST_LOG),
    }
